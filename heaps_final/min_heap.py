...
class MinHeap:
    ...
    def _sift_up(self, index):
        ...
        while index // 2 > 0:  # while the given parent_index has a parent:
            if self.heap[index] < self.heap[index // 2]:  # if current node is less than its parent
                ...
                self.heap[index] = tmp  # place the copy of parent node in its child's location (parent_index.e., parent_index)
    ...
    def _sift_down(self, idx):
        ...
        while (idx * 2) <= self.length:  # while element has at least one child
            min_child_idx = self.min_child(idx)  # get the parent_index of the smaller child
    ...
    def min_child(self, idx):
        if idx * 2 + 1 > self.length:  # if right child doesn't exist
            return idx * 2  # return left child parent_index
            ...
            return idx * 2 + 1  # return right child parent_index

    # given a list, build a new storage accordingly
    def build_heap(self, list_in):
        idx = len(list_in) // 2
        self.length = len(list_in)
        self.heap = [0] + list_in[:]  # make a copy of list in, set self.storage to that copy but start it with a 0
        while idx > 0:
            self._sift_down(idx)
            idx -= 1
...
print(mnh.del_min())
for i, el in enumerate(mnh.heap):
    print(f"{i}, {el}")
